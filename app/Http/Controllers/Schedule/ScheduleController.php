...
use Laratrust, DB, DataTables, Lang, DateTime;
...
class ScheduleController extends Controller
{
    ...
    private function generateScheduleForDay($id, $shifts, $availableAgents, $forecastedCallResults, $date)
    {
        foreach ($shifts as $shift) {
            ...
            // Define an array to keep track of hours worked by each agent
            $hoursWorkedPerAgent = [];

            // Assuming you have an array or collection of available agents
            foreach ($availableAgents as $agent) {
                $user = User::where('name', $agent->name)->first();

                // Check if the agent has worked a total of 40 hours or more already
                if (!isset($hoursWorkedPerAgent[$user->id]) || $hoursWorkedPerAgent[$user->id] < 40) {
                    // Calculate the hours worked by this agent so far
                    $hoursWorked = $this->calculateHoursWorked($user, $date);

                    // Calculate the remaining hours needed to reach 40 hours
                    $remainingHours = 40 - $hoursWorked;

                    if ($remainingHours <= 0) {
                        continue; // Skip this agent as they have already worked a full week
                    }

                    // Determine which shift to assign based on gender and available hours
                    if (
                        ($agent->gender === 'Male' && $shift->start_time > '15:00:00') ||
                        ($agent->gender === 'Female' && $shift->start_time < '15:00:00')
                    ) {
                        // Check if the agent has a shift on the same date
                        if (!$this->hasAssignedShiftOnSameDate($agent, $date)) {
                            // Check if the agent has a shift during the same time period
                            if (!$agent->hasAssignedShiftDuring($shift->start_time, $shift->end_time, $date)) {
                                // Calculate the duration of the shift
                                $shiftDuration = $this->calculateShiftDuration($shift->start_time, $shift->end_time);

                                // Ensure the agent doesn't exceed the remaining hours needed to reach 40
                                if ($shiftDuration <= $remainingHours) {
                                    ...
                                    // Update the hours worked for this agent
                                    $hoursWorkedPerAgent[$user->id] = isset($hoursWorkedPerAgent[$user->id]) ?
                                        $hoursWorkedPerAgent[$user->id] + $shiftDuration :
                                        $shiftDuration;

                                    // Reduce the count of agents needed
                                    $totalAgentsNeeded--;

                                    if ($totalAgentsNeeded <= 0) {
                                        break; // All required agents have been assigned
                                    }
                                }
                            }
                        }
    ...
    // Function to calculate the duration of a shift
    private function calculateShiftDuration($startTime, $endTime)
    {
        // Convert the start and end times to DateTime objects for easier calculations
        ...
        // Get the total hours and minutes as a float
        $hours = $interval->h + ($interval->i / 60);

        return $hours;
    }

    // Function to calculate the hours worked by an agent on a given date
    private function calculateHoursWorked($user, $date)
    {
        // Assuming you have a model for shifts, adjust this query to your data structure
        $shifts = ScheduleShift::where('agent_id', $user->id)
            ->where('date', $date)
            ->get();
        ...
        foreach ($shifts as $shift) {
            $totalHoursWorked += $this->calculateShiftDuration($shift->start_time, $shift->end_time);
        }
