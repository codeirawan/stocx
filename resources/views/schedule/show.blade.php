                        ...
                        <div class="kt-portlet__head-toolbar">
                            ...
                            @if (Laratrust::isAbleTo('create-schedule'))
                                ...
                                @if ($hasUnpublishedSchedule)
                                    <form id="publish-form" action="{{ route('schedule.publish', $scheduling->id) }}"
                                        method="POST" style="display: none;">
                                        @csrf
                                    </form>
                                ...
                                @else
                                    <form id="unpublish-form"
                                        action="{{ route('schedule.unpublish', $scheduling->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                    </form>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($getSchedules as $schedule)
                                            ...
                                            <tr class="schedule-row"
                                                data-filter="{{ $schedule->nik }} {{ $schedule->name }} {{ $schedule->gender }} {{ $schedule->religion }}">
                                                <td>{{ $schedule->nik }}</td>
                                                <td>{{ $schedule->name }}</td>
                                                <td>
...
// Check if it's time for a break
                                                ...
                                                <td>
                                                    <a href="#"
                                                        ...
                                                        title="{{ Lang::get('Swap') }}" data-toggle="modal"
                                                        data-target="#swapModal{{ $schedule->schedule_shift }}">
...
@foreach ($getSchedules as $schedule)
    <div class="modal fade" id="swapModal{{ $schedule->schedule_shift }}" tabindex="-1" role="dialog"
        aria-labelledby="swapModalLabel{{ $schedule->schedule_shift }}" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('schedule.swap', ['id' => $schedule->schedule_shift]) }}" method="POST"
                    id="swapSchedule" name="swapSchedule">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <p>Date: <span id="modalCurrentDate">{{ date('D, d M Y', strtotime($schedule->date)) }}</span>
                        </p>
                        <p>Current Shift: <span id="modalCurrentShift">{{ $schedule->shift }}</span></p>
                        <p>Choose Shift to Swap:</p>
                        <select class="form-control kt_selectpicker" id="swap" name="swap"
